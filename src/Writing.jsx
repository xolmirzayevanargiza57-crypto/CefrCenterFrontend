...
export default function Writing({ 
  ...
  const [testsState, setTestsState] = useState(tests);
  const [loading, setLoading] = useState(tests.length === 0);
  ...
  async function submit() {
    ...
    try {
      ...
      const pNum = part.partNum || (part.id.includes("P1") ? 1 : 3);
      const maxForThisPart = pNum === 1 ? 15 : (pNum === 2 ? 20 : 40);
  ...
  // ── LIST ────────────────────────────────────────────────────────────────────
  if (view === "list") {
    const hasTopics = writingTopics && (
      (writingTopics.part_1 && writingTopics.part_1.length > 0) ||
      (writingTopics.part_2 && writingTopics.part_2.length > 0)
    );

    return (
      <div style={{ animation: "fadeUp .4s ease", padding: "20px" }}>
        <style>{`
                      ...
                      <div key={t.id}
                        ...
                        <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
                          <div style={{ width: 38, height: 38, borderRadius: "50%", background: allDone ? "#EF9F27" : "rgba(255,255,255,0.05)", border: allDone ? "none" : "1px solid rgba(255,255,255,0.1)", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                            {allDone ? <Ic n="check" s={18} c="#fff" /> : <span style={{ fontSize: 13, fontWeight: 800, color: "#8b9bbf" }}>{testsState.indexOf(t) + 1}</span>}
                          </div>
                          <div>
            ...
            {hasTopics && (
              <div>
                ...
                >
                  ...
                  {showTopics ? "Hide" : "Show"} All Topics ({ (writingTopics.part_1?.length || 0) + (writingTopics.part_2?.length || 0) } topics)
                </button>
  ...
  if (view === "test" && part) {
    ...
    return (
      <div style={{ animation: "fadeUp .3s ease", maxWidth: 700, margin: "0 auto", padding: "20px" }}>
        <style>{`
          @keyframes fadeUp{from{opacity:0;transform:translateY(12px)}to{opacity:1;transform:translateY(0)}}
